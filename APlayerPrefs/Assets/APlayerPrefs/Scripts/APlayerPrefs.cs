...
namespace Dands.APlayerPrefs
{
    ...
    {
        private static bool AUTO_SAVE = APlayerPrefsSettings.Instance.GetAutoSave();
        private static float TIME_AUTO_SAVE = APlayerPrefsSettings.Instance.GetTimeToAutoSave();
        private static WaitForSeconds DELAY_SAVE;
        private static string SAVE_FILE_NAME = APlayerPrefsSettings.Instance.GetNameFile();
        ...
        private static string GetDictionary(string key)
        {
            ...
            if (!HasKey(key))
            {
                Debug.Log($"KEY {key} not fount, return default valeu");
            }
            return value;
        }
        #endregion
